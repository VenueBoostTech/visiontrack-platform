      ...
      try {
        if (user.ownedBusiness.vtCredentials && cameras) {
          ...
          cameras.forEach(async (camera: any, index: number) => {
            ...
            const startResponse: any = await VTCameraService.startCameraStream(
              camera.id
            );
            newCameras[index] = {
              ...newCameras[index],
              stream_url: `${process.env.VISION_TRACK_API}${startResponse.stream_url}`,
            };
          });
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedCamera(
                e.target.value === "1x1" ? allCameras[0] : null
              );
              e.target.value === "2x2"
                ? setCurrentCameras(allCameras.slice(0, 4))
                : e.target.value === "3x3"
                  ? setCurrentCameras(allCameras.slice(0, 9))
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-3">
          <Card className={`grid ${getGridLayout()} gap-4 aspect-[16/9]`}>
            {isClient &&
              Array.from({
                length: parseInt(layout[0]) * parseInt(layout[2]),
              }).map((_, index) => (
