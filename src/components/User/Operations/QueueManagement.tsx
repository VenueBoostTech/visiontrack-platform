...
const mockData = {
  queueStatus: [
    { register: 'R1', waitTime: 4.5, customers: 6, efficiency: 85, status: 'high' },
    { register: 'R2', waitTime: 2.2, customers: 3, efficiency: 92, status: 'normal' },
    { register: 'R3', waitTime: 6.8, customers: 8, efficiency: 78, status: 'critical' },
    { register: 'R4', waitTime: 1.5, customers: 2, efficiency: 95, status: 'normal' },
  ],
  waitTimeTrend: [
...
export default function QueueManagement() {
    ...
    const avgWaitTime = mockData.queueStatus.reduce((acc, curr) => acc + curr.waitTime, 0) / mockData.queueStatus.length;
    const totalCustomers = mockData.queueStatus.reduce((acc, curr) => acc + curr.customers, 0);
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <h3 className="text-2xl font-bold">{avgWaitTime.toFixed(1)}m</h3>
                  ...
                  <div key={zone.zone} className="p-4 border rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        ...
                        {Math.round((zone.current / zone.required) * 100)}% coverage
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(zone.current / zone.required) * 100}%` }}
                      />
                    </div>
                  </div>
                ))}
